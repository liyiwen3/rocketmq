...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    private ServiceState serviceState = ServiceState.CREATE_JUST;//生产者的状态
    private MQClientInstance mQClientFactory;//保存进程中所有的客户端，不能有两个相同的group的生产者和消费者
        ...
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                if (!this.defaultMQProducer.getProducerGroup().equals(MixAll.CLIENT_INNER_PRODUCER_GROUP)) {
                    this.defaultMQProducer.changeInstanceNameToPID();//实例名如果是DEFAULT，则将实例名改成pid
                }

                this.mQClientFactory = MQClientManager.getInstance().getAndCreateMQClientInstance(this.defaultMQProducer, rpcHook);//获取或者创建客户端工厂

                boolean registerOK = mQClientFactory.registerProducer(this.defaultMQProducer.getProducerGroup(), this);//往客户端工厂注册生产者
                if (!registerOK) {//如果同一个group 生产者已存在则抛出异常
                    this.serviceState = ServiceState.CREATE_JUST;//在一个进程中不允许有两个相同group的生产者
                ...
                if (startFactory) {
                    mQClientFactory.start();//启动客户端工厂
                }

                log.info("the producer [{}] start OK. sendMessageWithVIPChannel={}", this.defaultMQProducer.getProducerGroup(),
                    this.defaultMQProducer.isSendMessageWithVIPChannel());
                this.serviceState = ServiceState.RUNNING;//生产者已启动运行
                break;
            case RUNNING:
            case START_FAILED:
